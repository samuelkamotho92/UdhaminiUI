...
function AllPremium() {
  ...
  const [successMsg, setSuccessMgs] = useState(null);
  ...
  const { data: myData, isLoading, error: fetchError } = useQuery(["AllPremium"], () =>
    wretch(`${DeployedURL}/scholarship/premium`)
      .headers({ token: `Bearer ${authToken}` })
      .post({ premium_tier: true }).json().then((data) => { return data; })
  ...
  const DeleteScholarshipMutation = useMutation({
    mutationFn: async (id) => {
      wretch(`${DeployedURL}/scholarship/delete/${id}`)
        .headers({ token: `Bearer ${authToken}` }).delete().json()
        .then((data) => {
          setSuccessMgs("Scholarship deleted successfully");
    ...
    mutationFn: async (data) => {
      wretch(`${DeployedURL}/scholarship/update`)
        ...
        .put(data).json().then((data) => {
          queryClient.invalidateQueries('AllPremium');
          setSuccessMgs("Scholarship updated successfully");
          setTimeout(() => setSuccessMgs(false), 3000);
          reset();
        })
          ...
          {isLoading ? (
            ...
            (
              ...
              fetchError ? (
                ...
                successMsg && (
                  <div className="alert alert-info mt-60px shadow-lg w-fit z-50 text-center text-white absolute top-0 right-0" >
                    <div><span className='text-2xl'>😁</span>
                  ...
                  <tbody>
                    {myData?.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>{item.link}</td>
                        <td >
                          <label htmlFor="my-modal-3" className="btn btn-info btn-sm btn-outline" id={item._id} onClick={() => setFormData(item)}>
                            <FaPencilAlt />
                          </label>
        ...
        <div className="modal-box relative">
          ...
          <form onSubmit={handleSubmit(updateSubmit)}>
            ...
            <div className="form-control mt-6">
              <button type="submit" className="btn btn-outline  btn-warning w-3/4">Update</button>
            </div>
          </form>
        </div>
      </div>
